...
| 字段 | 默认值 | 说明 |
| --- | --- | --- |
| `remotePublishMode` | `githubPackages` | 远程发布模式。`githubPackages` 使用 GitHub Packages；`central` 使用 Sonatype Central Portal；`customRepository` 使用旧自定义 Maven 仓库。 |
...
```kotlin
PublishInfo {
    remotePublishMode = "githubPackages" // githubPackages / central / customRepository
...
| --- | --- | --- | --- |
| 发布模式 | `-PremotePublishMode=githubPackages` | `REMOTE_PUBLISH_MODE` | 可选 `githubPackages` / `central` / `customRepository`。 |
