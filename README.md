...
| Android Library 远程发 Central | `groupId`、`artifactId`、`version`、`pomDescription`、`pomUrl` | 默认 namespace 和开发者信息面向 Entertech；非 `cn.entertech` namespace 必须覆盖 `centralNamespace`。通过 Gradle property 或环境变量传入 Central token 和 GPG signing 信息。`version` 不能包含 `debug`。 |
...
- Central、POM、License、Developer、SCM 字段支持命令行覆盖：`Gradle property > 环境变量 > PublishInfo 字段 > 默认/推导值`。
...
| `scmConnection` | 只读 SCM 地址，可由 `scmUrl` 推导 | `scm:git:git://github.com/Entertech/PublishPlugin.git` |
