...
本仓库的发布身份集中维护在根目录 `gradle.properties` 的 `PublishPlugin distribution identity` 区块。fork 后优先修改这里：

```properties
publishGroup=io.github.wk1995.android
publishArtifactId=publish
publishPluginId=wk1995.android.publish
publishVersion=1.2.1
publishCentralNamespace=io.github.wk1995
```

`plugin_base/build.gradle.kts`、发布校验脚本和 README 插件坐标同步脚本都会读取这组配置。常规 fork 只需要替换 group、artifact、plugin id、version、POM/SCM/developer 字段和 Central namespace，不需要分散修改多个脚本。
...
buildscript {
    ...
    dependencies {
        classpath("io.github.wk1995.android:publish:1.2.1")
    }
}
```
...
jobs:
  publish:
    uses: wk1995/PublishPlugin/.github/workflows/central-publish.yml@main
    secrets: inherit
    with:
      module: ":library"
      namespace: "io.github.wk1995"
...
```text
io.github.wk1995.android:publish:1.2.1
wk1995.android.publish:wk1995.android.publish.gradle.plugin:1.2.1
```
...
```kotlin
plugins {
    id("wk1995.android.publish") version "1.2.1"
}
```
