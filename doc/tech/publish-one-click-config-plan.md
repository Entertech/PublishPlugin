...
采用方案 1：**Gradle task 形式的一键发布配置 CLI**。
...
```bash
./gradlew :library:ConfigurePublishTask
./gradlew :library:ConfigurePublishTask
```
...
```bash
./gradlew :library:RollbackPublishSecretsTask
./gradlew :library:RollbackPublishSecretsTask
```
...
```bash
./gradlew :library:GeneratePublishConfigTask
./gradlew :library:GeneratePublishConfigTask
```
...
```bash
./gradlew :demo-lib:configurePublish
./gradlew :demo-plugin:configurePublish
```
...
- 不设计 `configurePublishAll`，多模块仓库按模块分别执行 task。
...
| 配置类别 | 一键发布配置行为 | 是否进入版本控制 |
| --- | --- | --- |
| 组件发布元数据 | 由目标模块 `build.gradle.kts` / `build.gradle` 中的 `PublishInfo` 维护；一键任务只校验，不写入、不覆盖 | 是 |
| 本地 `local.properties` | 默认配置落点；追加 `publish.*` key，不覆盖已有 `sdk.dir` / `ndk.dir`；发布运行时作为仓库级通用字段 fallback；task 负责确保文件被 ignore | 否，不能被 git track |
...
`local.properties` 是 Android 项目的本地配置约定，通常已有 `sdk.dir` 等本机路径，也通常已经在 Android 模板的 `.gitignore` 中。一键发布配置默认把 `publish.*` 配置块追加到根目录 `local.properties`；如果文件不存在就创建；如果 `.gitignore` 缺少 `local.properties`，task 自动补充；如果 `local.properties` 已经被 git track，task 必须失败并提示先移出版本控制。
...
```text
Gradle property > 环境变量 > PublishInfo 显式配置 > local.properties 中 publish.* 非空值 > 插件默认值/推导值
```
...
- `local.properties` 中优先读取 `publish.*` key；旧 `centralPublish.*` key 只做兼容读取，且同名字段被 `publish.*` 覆盖。已有 `sdk.dir` / `ndk.dir` 等 Android 本地字段会被保留并忽略。
- `local.properties` 中 value 为空的 `publish.*` key 会被忽略，等同于没有配置。
- `local.properties` 不支持 `publish.groupId`、`publish.artifactId`、`publish.version`、`publish.pluginId`、`publish.implementationClass`、`publish.pomName`、`publish.pomDescription`、`publish.pomUrl` 等组件字段；发现这些字段时任务失败并提示移动到对应模块 `PublishInfo`。
...
- `./gradlew :demo-lib:configurePublish` 只处理 `:demo-lib`。
- `./gradlew :demo-plugin:configurePublish` 只处理 `:demo-plugin`。
...
默认读取仓库根目录的 `local.properties`。Android 项目通常已经有这个文件，一键发布配置只追加 `publish.*` 配置块，不改写已有的 `sdk.dir` / `ndk.dir`。也可以通过 `-PpublishConfig=...` 指定相对路径或绝对路径；自定义文件仍建议使用同一套 `publish.*` key，并由 task 自动检查 ignore/track 状态。
...
如果 `local.properties` 已存在，生成器保留原内容，只追加或更新 `publish` 配置块；如果文件不存在，则创建一个只包含该配置块的 `local.properties`。生成模板里的注释使用 ASCII English，避免 IDE 按 `.properties` 默认编码打开时出现中文注释乱码。已有旧模板需要刷新注释时，可以传入 `-PoverwritePublishConfig=true`，task 会重写模板注释，并把旧 `centralPublish.*` value 转写到新的 `publish.*` key。生成内容示例：
...
publish.githubActions=

# GitHub Actions workflow file path. Blank uses the current module name, for example .github/workflows/publish-demo-lib.yml.
publish.workflowPath=

# Reusable workflow reference, for example Entertech/PublishPlugin/.github/workflows/publish.yml@main.
publish.workflowUses=
...
publish.scmUrl=https://github.com/Entertech/demo-lib
...
publish.gpgKeyFile=/Users/chengpeng/secrets/gpg-private-key.asc
publish.signingPassword=gpg-private-key-password
publish.signingKeyId=
publish.gpgGenerate=false
publish.gpgKeyType=RSA
publish.gpgKeyLength=4096
publish.gpgKeyExpire=2y
publish.gpgName=Entertech
publish.gpgEmail=developer@entertech.cn
publish.gpgComment=Central Publish Signing Key
...
下表展示逻辑字段名；写入 `local.properties` 时统一使用 `publish.<字段名>`，例如 `publish.mavenCentralUsername=`。
...
| 兼容字段 | 等价字段 |
| --- | --- |
| `publish.centralUsername` | `publish.mavenCentralUsername` |
| `publish.centralPassword` | `publish.mavenCentralPassword` |
| `publish.signingInMemoryKeyPassword` | `publish.signingPassword` |
| `publish.signingInMemoryKeyId` | `publish.signingKeyId` |

secret 名称允许覆盖：

```properties
publish.mavenCentralUsernameSecret=MAVEN_CENTRAL_USERNAME
publish.mavenCentralPasswordSecret=MAVEN_CENTRAL_PASSWORD
publish.gpgKeySecret=GPG_KEY_CONTENTS
publish.signingPasswordSecret=SIGNING_PASSWORD
publish.signingKeyIdSecret=SIGNING_KEY_ID
```
...
2. 在读取敏感字段前检查配置文件安全：
   - 配置文件必须存在；不存在时提示先运行 `:module:generatePublishConfig`。
...
4. 如果配置文件包含 `publish.groupId` 等组件字段，任务失败并提示移动到当前模块 `PublishInfo`。
5. 使用当前 Gradle project 作为目标模块，合并当前模块 `PublishInfo`、配置文件仓库级 fallback、插件默认值，得到有效发布信息。
6. 校验 PublishInfo 必填坐标字段：
   - `groupId`
   - `artifactId`
   - `version`
7. 解析 `publishTarget`，默认 `github_packages`；显式支持 `central` 和 `all`。
8. 当 `publishTarget=central/all` 时校验 Central 规则：
   - `version` 不能包含 `debug`。
...
9. 如果 `githubSecrets=true` 且 `publishTarget=central/all`：
   - 检查 `gh` 是否存在。
   - 执行 `gh auth status`。
   - 解析或校验 `githubRepo`。
...
10. 如果 `githubActions=true`：
   - 生成或更新 `workflowPath` 指定的文件；为空时按当前模块名生成 `.github/workflows/publish-<module>.yml`。
11. 打印结果摘要：
   - 校验了哪个模块。
   - 写入了哪些 secret 名称。
   - 生成了哪个 workflow。
...
单模块任务 `:module:configurePublish` 只配置当前模块。多模块仓库重复执行各模块自己的 task，不通过配置文件声明模块列表，也不注册 `configurePublishAll`。
...
一键发布配置任务支持 `gpgGenerate=true` 的非交互式生成模式。实现时应使用临时 batch 文件调用 `gpg --batch --generate-key`，生成后立刻导出 ASCII 私钥到 `gpgKeyFile`。临时 batch 文件必须写入系统临时目录，执行完成后删除。任务日志不得打印 `signingPassword` 或私钥内容。
...
1. 解析 `-PpublishConfig`，默认 `local.properties`。
2. 如果文件不存在，创建 `local.properties` 并写入完整 `publish.*` key 模板。
3. 模板注释必须使用 ASCII-only English，避免 `.properties` 文件在 IDE 或 Java properties 语义下显示乱码。
4. 如果文件已存在，保留 `sdk.dir` / `ndk.dir` 和其他已有内容，只追加缺失的 `publish.*` key；已存在的非空 value 默认不覆盖。
5. 用户传入 `-PoverwritePublishConfig=true` 时，允许重写已存在的 `publish.*` 模板注释并保留已有 value，不能删除 `sdk.dir` / `ndk.dir` 等非 `publish.*` 字段。
...
模板生成器不支持 `-PpublishModules` 或 `-PpublishModuleAliases`。多模块仓库使用同一份仓库级配置文件，然后分别在目标模块执行 `generatePublishConfig` 或 `configurePublish`。

### rollbackPublishSecrets

执行顺序：

1. 加载同一个 `local.properties`，优先读取 `publish.*` 配置，并兼容旧 `centralPublish.*`。
2. 解析 `publish.githubRepo`，为空时尝试通过 `gh repo view` 或 `git remote origin` 推导。
...
一键发布配置任务不自动修改 `build.gradle.kts` 或 `build.gradle`。如果当前模块没有应用 `cn.entertech.publish`，任务失败并提示用户先接入插件。原因是 plugins block 可能存在 version catalog、buildscript classpath、旧 Groovy apply 等多种写法，自动改动风险高。
...
除了 `configurePublish` 会读取 `local.properties` 外，正常发布任务也会读取该文件中的 `publish.*` 作为仓库级 fallback。这样业务仓库可以把仓库级通用字段集中放在本地配置文件中，组件级字段继续留在各模块 `PublishInfo`。
...
- `PublishConfigResolver` 增加 `loadPublishProperties(project)`，默认读取 root `local.properties`，并支持 `-PpublishConfig=...`。
- 解析 `publish.*` 前缀字段，兼容读取旧 `centralPublish.*`；`sdk.dir` / `ndk.dir` 等本地 Android 字段必须忽略。
...
- `centralUsername` / `centralPassword` 和 GPG signing 不从 `local.properties` 参与本地发布解析，避免用户误以为可以把敏感信息留在本地配置文件中长期使用；这两个类别只由 `configurePublish` 用来写 GitHub secrets。
...
```properties
publish.githubActions=true
publish.workflowPath=.github/workflows/publish-library.yml
publish.workflowUses=Entertech/PublishPlugin/.github/workflows/publish.yml@main
```
...
如果配置 `publish.publishTarget=central`，生成的 `with` 会增加 Central 参数：
...
```bash
./gradlew :demo-lib:configurePublish \
  -PworkflowPath=.github/workflows/publish-demo-lib.yml

./gradlew :demo-plugin:configurePublish \
  -PworkflowPath=.github/workflows/publish-demo-plugin.yml
```

如果 workflow 文件已存在：

- 如果文件包含标记 `# Generated by PublishPlugin configurePublish`，允许覆盖。
...
如果 `local.properties` 被 git track，`configurePublish` 必须失败，不写 secret，不写 workflow。即使当前 value 为空，也要先移出版本控制，避免后续填入敏感信息后误提交。
...
这里不要求用户手工编辑 `.gitignore`。`generatePublishConfig` / `configurePublish` 会在发现缺少 `local.properties` ignore 规则时自动追加；如果文件已经被 git track，只能由用户确认后执行 `git rm --cached`，task 不自动修改暂存区。
...
- 重新运行 `configurePublish` 写入新的 repository secrets。
- 清理 GitHub Actions logs / artifacts。
- 通知协作者重新 clone 或按团队约定 reset 到新历史。

`rollbackPublishSecrets` 可以删除 GitHub repository secrets 和提示历史清理命令，但默认不自动执行 history rewrite。如果要输出完整 history rewrite 指南，使用显式危险开关：
...
| 文件 | 责任 |
| --- | --- |
| `plugin_base/src/main/kotlin/custom/android/plugin/config/PublishConfig.kt` | properties 配置模型和字段归一化。 |
| `plugin_base/src/main/kotlin/custom/android/plugin/config/PublishConfigLoader.kt` | 读取配置文件、解析相对路径、校验必填字段。 |
| `plugin_base/src/main/kotlin/custom/android/plugin/config/PublishConfigTemplateWriter.kt` | 生成 key 完整、value 为空且包含字段注释的 `publish.*` 配置块；写入 `local.properties` 时保留已有 `sdk.dir` / `ndk.dir`。 |
...
| `plugin_base/src/main/kotlin/custom/android/plugin/GeneratePublishConfigTask.kt` | 生成本地配置模板 task。 |
| `plugin_base/src/main/kotlin/custom/android/plugin/ConfigurePublishTask.kt` | 一键发布配置入口 task。 |
| `plugin_base/src/main/kotlin/custom/android/plugin/RollbackPublishSecretsTask.kt` | repository secrets 回退 task。 |
...
| `plugin_base/src/main/kotlin/custom/android/plugin/PublishPlugin.kt` | 注册 `generatePublishConfig` / `GeneratePublishConfigTask` / `configurePublish` / `ConfigurePublishTask` / `rollbackPublishSecrets` / `RollbackPublishSecretsTask`。 |
| `README.md` | 增加一键发布配置使用说明。 |
| `doc/tech/publish-one-click-config-plan.md` | 本方案文档。 |

测试文件：

| 文件 | 覆盖点 |
| --- | --- |
| `plugin_base/src/test/java/custom/android/plugin/config/PublishConfigLoaderTest.java` | properties 解析、只读取 `publish.*`、忽略 `sdk.dir` / `ndk.dir`、兼容字段、必填字段校验。 |
| `plugin_base/src/test/java/custom/android/plugin/config/PublishConfigTemplateWriterTest.java` | 模板生成、字段注释、枚举说明、空值保留、已有 `local.properties` 保留 `sdk.dir` / `ndk.dir`、缺少 ignore 规则时自动追加。 |
...
| `plugin_base/src/test/java/custom/android/plugin/ConfigurePublishTaskFunctionalTest.java` | TestKit 端到端配置模块、生成 workflow、调用 fake `gh`。 |
| `plugin_base/src/test/java/custom/android/plugin/RollbackPublishSecretsTaskFunctionalTest.java` | 删除 secrets、配置文件被 track 时的提示。 |
...
- [ ] 新增 `PublishConfig` 数据类。
- [ ] 新增 `PublishConfigLoader`。
- [ ] 测试优先读取 `publish.*` 前缀字段，并兼容旧 `centralPublish.*`，忽略 `sdk.dir` / `ndk.dir`。
- [ ] 测试无前缀的 `githubRepo` / `centralNamespace` 不会被当作有效配置读取，并提示使用 `publish.*`。
...
- [ ] 新增 `PublishConfigTemplateWriter`。
- [ ] 新增 `GeneratePublishConfigTask`。
...
- [ ] 测试 `local.properties` 已存在时保留 `sdk.dir` / `ndk.dir` 和其他非 `publish.*` 内容。
- [ ] 测试文件已存在且已有非空 `publish.*` value 时不覆盖。
...
- [ ] 测试本地配置文件中 `publish.pomUrl=` 会失败并提示移动到 `PublishInfo`。
...
### Task 4：configurePublish functional test

- [ ] 构造 TestKit fixture，模块应用 `cn.entertech.publish`。
- [ ] 准备 `local.properties`。
- [ ] 使用 fake `gh` 跑 `:fixture:configurePublish`。
...
- [ ] 验证默认 `.github/workflows/publish-fixture.yml` 生成，或 `-PworkflowPath=...` 指定路径时生成指定文件。
- [ ] 验证 `publishTarget=central/all` 时 fake `gh` 收到 4 个必需 secret。
...
### Task 5：rollbackPublishSecrets functional test

- [ ] 用 fake `gh` 跑 `:fixture:rollbackPublishSecrets`。
- [ ] 验证删除默认 secret 名称。
...
- [ ] README 增加按模块执行的一键发布配置命令、配置文件 fallback 优先级、执行命令、回退命令、泄露处理说明。
- [ ] 跑 `./gradlew :plugin_base:test :demo-lib:test :demo-plugin:test --console=plain`。

## 验收标准

1. 业务仓库执行 `:module:generatePublishConfig` 能生成或更新根目录 `local.properties`，保留 Android 已有 `sdk.dir` / `ndk.dir`，并自动确保 `.gitignore` 包含 `local.properties`。
2. 多模块仓库不需要 `modules` 字段；分别执行 `:demo-lib:configurePublish`、`:demo-plugin:configurePublish` 时，每个 task 只处理当前模块，并复用同一份仓库级通用配置。
3. `PublishInfo` 最小只需要 `groupId`、`artifactId`、`version`；`pomName`、`pomDescription`、`pomUrl` 能通过默认值和 Git/CI 推导得到。
4. 执行 `:module:configurePublish` 能完成单模块 `PublishInfo` 校验和 workflow 生成；`publishTarget=central/all` 时额外写入 Central/GPG GitHub secrets。
...
13. `rollbackPublishSecrets` 能删除对应 repository secrets。
