...
多 variant 同时发布不应该把 `product`、`authentication` 这类业务维度硬编码进发布插件。插件只负责识别 Android release component，并给业务脚本提供当前 variant 信息；具体 artifactId 命名规则由业务项目通过 `PublishInfo` 动态声明。

### 设计目标

1. 兼容旧字段：`PublishInfo.artifactId` 仍是必填字段，也是默认 artifactId。
2. 单 release component 项目保持旧行为：publication 名继续是 `EnterPublish`，artifactId 继续使用 `PublishInfo.artifactId`。
3. 多 release variant 项目支持一次生成多个 publication：publication 名为 `<VariantName>EnterPublish`，例如 `BreathAuthReleaseEnterPublish`。
4. artifactId 不内置固定 product/auth 规则，而是通过可选回调动态计算。
5. 未配置回调时，所有 publication 仍使用 `PublishInfo.artifactId`，避免旧项目行为突变。
...
`PublishLibraryRemoteTask` 继续作为唯一远程发布入口：

- 单 publication：保持旧任务路径，执行 `publishEnterPublishPublicationTo<RepositoryName>Repository`。
- 多 publication：执行 `publishAllPublicationsTo<RepositoryName>Repository`，一次发布所有 release variant。
- 自定义仓库模式下 repository name 仍为 `Maven`；Central 模式下使用 `centralRepositoryName`，默认 `CentralStaging`。
